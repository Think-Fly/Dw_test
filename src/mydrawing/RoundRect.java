...
class RoundRect extends DrawingPanel{
	private int InitX;
	private int InitY;
	private int PreX;
	private int PreY;
	
	public RoundRect(BufferedImage[] ImageArray, JLabel mLabel){
		...
		addMouseListener(new MouseAdapter(){
			...
			public void mouseReleased(MouseEvent e){
				drawRoundRect(InitX, InitY, e.getX(), e.getY());
				InitX = -1;
				InitY = -1;
			}
		});
	...
	public void paint(Graphics g){
		...
		if(InitX > 0 && InitY > 0){
			g2d.drawRoundRect(Math.min(InitX, PreX), Math.min(InitY, PreY),
