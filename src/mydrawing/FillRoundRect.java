...
*/
class FillRoundRect  extends DrawingPanel{
	private int InitX;
	private int InitY;
	private int PreX;
	private int PreY;
	
	public FillRoundRect(BufferedImage[] ImageArray, JLabel mLabel){
		...
		addMouseListener(new MouseAdapter(){
			...
			public void mouseReleased(MouseEvent e){
				drawFillRoundRect(InitX, InitY, e.getX(), e.getY());
				InitX = -1;
				InitY = -1;
			}
		});
	...
	public void paint(Graphics g){
		...
		if(InitX > 0 && InitY > 0){
			g2d.fillRoundRect(Math.min(InitX, PreX), Math.min(InitY, PreY),
					Math.abs(InitX - PreX), Math.abs(InitY - PreY), 25, 25);
		}
		else
			g2d.drawString("", 1, 1);
	}
